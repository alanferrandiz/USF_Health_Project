...
var pools = ["P0","P1","P2","P3","P4","P5","P6","P7","P8","P9","P10"];

function retrieveInvidualData (e,txtBarcode,txtIndividual) {
  
  var value = txtBarcode.value;
  ...
  if(e.keyCode === 13){
    ...
    if (isNaN(value.substr(1,7))) {
      txtIndividual.value = '';
      txtBarcode.value = value.substr(0,8);
    }
    else {
        var barcode = Number(value.substr(1,7));
     
        var valuefound = countries[barcode];
      
   

        if (valuefound != undefined) {
          txtIndividual.value = countries[barcode];
          txtBarcode.value = value.substr(0,8);
        }
        else {
          txtIndividual.value = '';
...
function autocomplete(inp, sel, par, prefix) {
        ...
        if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
          ...
          b.addEventListener("click", function(e) {
            ...
            if (par == 1) {
              ...
              inp.value = sel.value  + " | " + this.getElementsByTagName("input")[0].value + ""; 
            }
            else if (par == 2) {
              sel.value =   this.getElementsByTagName("input")[0].value;
              inp.value = sel.value;
            }
              closeAllLists();
          });
